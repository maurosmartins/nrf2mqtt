...
#Credit to Brian Lavery at:
#https://github.com/BLavery/lib_nrf24/tree/master
...
class NRF24:
    ...
    # Bit positions Mnemonics use function _BV() to create mask*/
    MASK_RX_DR = 6
    MASK_TX_DS = 5
    MASK_MAX_RT = 4
    EN_CRC = 3
    ...
    def read_payload(self, buf_len):
        txbuffer = [NRF24.NOP for i in range(0, buf_len+1)]
        txbuffer[0] = NRF24.R_RX_PAYLOAD
        payload = self.spidev.xfer2(txbuffer)
        return payload[1:]

    def vNRFGetPayload(self, buf_len=-1):

        txbuffer = [NRF24.NOP for i in range(0, buf_len)]
        txbuffer[0] = NRF24.R_RX_PAYLOAD
    ...
    def print_byte_register(self, name, reg, qty=1):
        print ("%s = 0x%02x" % (name, self.read_register(reg)) )
    ...
    def printDetails(self):
        self.print_byte_register("STATUS", NRF24.STATUS)
        ...
        self.print_address_register("RX_ADDR_P0", NRF24.RX_ADDR_P0, 2)
        self.print_address_register("RX_ADDR_P1", NRF24.RX_ADDR_P1, 2)
        self.print_byte_register("RX_ADDR_P2", NRF24.RX_ADDR_P2, 4)
        self.print_byte_register("RX_ADDR_P3", NRF24.RX_ADDR_P3, 4)
        self.print_byte_register("RX_ADDR_P4", NRF24.RX_ADDR_P4, 4)
        self.print_byte_register("RX_ADDR_P5", NRF24.RX_ADDR_P5, 4)
    ...
    def begin(self, csn_pin, ce_pin=0):   # csn & ce are RF24 terminology. csn = SPI's CE!
        ...
        self.speed = 1000000
        self.spidev.max_speed_hz=self.speed
    ...
    def bNRFIsDataReady(self):
        ...
        if result:
            self.write_register(NRF24.STATUS, (status | _BV(NRF24.RX_DR) ))
        return result
    ...
    def ConvertFromIntListToString(self, IntList): #{
        s=''
        for l in IntList:
            s = s + chr(l)
        return s
    ...
    def bNRFWasAckReceived(self): 
        u8Status = self.read_register(NRF24.STATUS)
        self.print_byte_register("STATUS", NRF24.STATUS)
        if ((u8Status&_BV(self.MASK_TX_DS)) == _BV(self.MASK_TX_DS)):
            self.write_register(NRF24.STATUS, _BV(self.MASK_TX_DS))
            return True
        else:
            return False
            
            
    def vNRFSendPayload(self, buf):
        self.write_register(NRF24.STATUS, 0b01111110)
        self.write_register(NRF24.CONFIG,0x3A)
        self.flush_tx()
        
        txbuffer = bytes([NRF24.W_TX_PAYLOAD])
        txbuffer+=buf
    ...
    def enableTxWithoutAck(self):
        # Enable Tx without requesting an Ack
        self.write_register(NRF24.FEATURE, self.read_register(NRF24.FEATURE) | _BV(NRF24.EN_DYN_ACK))
        
    def enableDynPayloads(self):    
        # Enable dynamic payload throughout the system
        self.write_register(NRF24.FEATURE, self.read_register(NRF24.FEATURE) | _BV(NRF24.EN_DPL))
        
    def vNRFSendPayloadNoAck(self, buf):
        self.write_register(NRF24.STATUS, 0b01111110)
        self.write_register(NRF24.CONFIG,0x3A)
        self.flush_tx()
        
        txbuffer = bytes([NRF24.W_TX_PAYLOAD_NO_ACK])
        txbuffer+=buf
